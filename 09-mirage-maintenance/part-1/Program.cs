...
foreach(string line in input)
{
  ...
  extrapolatedSum += ExtrapolateNextValue(history);
}

Console.WriteLine($"The sum of all extrapolated values is {extrapolatedSum}");

long ExtrapolateNextValue(List<long> values)
{
  if(values.All(v => v == 0))
  {
    return 0;
  ...
  return values[values.Count - 1] + ExtrapolateNextValue(secondaryValues);
}